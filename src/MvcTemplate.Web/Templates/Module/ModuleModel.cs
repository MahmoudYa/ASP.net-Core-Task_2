...
public class ModuleModel
{
    ...
    public ModuleModel(String model, String controller, String? area)
    {
        ...
        Type modelType = assembly.GetType($"Temp.Objects.{model}") ?? typeof(AModel);
        Type viewType = assembly.GetType($"Temp.Objects.{model}View") ?? typeof(AView<>);
        BindingFlags declaredOnly = BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly;

        ControllerNamespace = $"Temp.Controllers.{area}".Trim('.');
        ...
        {
            Type type = assembly.GetType($"Temp.Objects.{model}{view}View") ?? viewType;
